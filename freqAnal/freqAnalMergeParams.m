function [mergedParams,mergedData] = freqAnalMergeParams(groupName,...
  oldParams,newParams,oldData,newData)
%
% [mergedParams,mergedData] = freqAnalMergeParams(groupName,oldParams,newParams,[oldData],[newData])
%
% Merges freqAnal params. Called by saveOverlay or saveAnalysis when file
% already exists and 'merge' option is selected. Calls
% freqAnalReconcileParams to make sure that the merged params and data are
% consistent with the tseries files.
%
% oldParams: freqAnal params structure in saved file.
% newParmas: freqAnal params structure freqresponding to newly computed
%            analysis.
% oldData: cell array of data arrays freqresponding to oldParams.
%            Default: []
% newData: cell array of data arrays freqresponding to newParams.
%            Default: []
%
...
%[oldParams,oldData] = freqAnalReconcileParams(groupName,oldParams,oldData);
%[mergedParams,mergedData] = freqAnalReconcileParams(groupName,newParams,newData);
